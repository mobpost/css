/* mobcodecolor ver 1.21 by mobschools.com */
function mobCodeColor() {
...
for (j = 0; j < k; j++) {
    ...
    l = x.length;
    for (i = 0; i < l; i++) {
        x[i].innerHTML = mobCodeColorize(x[i].innerHTML, modes[j]);
    }
}
}
function mobCodeColorize(x, lang) {
  ...
  function htmlMode(txt) {
    ...
    php = new extract(rest, "&lt;\\?php", "?&gt;", phpMode, "MOBPHPPOS");
    rest = php.rest;
    comment = new extract(rest, "&lt;!--", "--&gt;", commentMode, "MOBHTMLCOMMENTPOS");
    rest = comment.rest;
    ...
    for (i = 0; i < comment.arr.length; i++) {
        rest = rest.replace("MOBHTMLCOMMENTPOS", comment.arr[i]);
    }
    for (i = 0; i < php.arr.length; i++) {
        rest = rest.replace("MOBPHPPOS", php.arr[i]);
    }
    return rest;
  }
  ...
  function cssMode(txt) {
    ...
    comment = new extract(rest, /\/\*/, "*/", commentMode, "MOBCSSCOMMENTPOS");
    rest = comment.rest;
    while (rest.search("{") > -1) {
      s = rest.search("{");
    ...
    rest = rest.replace(/}/g, "<span style=color:" + cssdelimitercolor + ">}</span>");
    for (i = 0; i < comment.arr.length; i++) {
        rest = rest.replace("MOBCSSCOMMENTPOS", comment.arr[i]);
    }
  ...
  function jsMode(txt) {
    ...
    for (i = 0; i < esc.length; i++) {
      rest = rest.replace("MOBJSESCAPE", esc[i]);
    }
